...
상대 속도와 상대 가속도가 완전히 0이 되어 특정 평형 위상각에 고정되는 오실레이터들입니다 ($\dot{\phi}_i = \ddot{\phi}_i = 0$). 
위상 고정 평형 조건식은 다음과 같습니다.
...
2차 쿠라모토 모델의 가장 핵심적인 수학적·물리적 특성은 1차 모델의 연속 상전이와 달리 **1차 불연속 상전이(1st-order Discontinuous Phase Transition)** 
와 **히스테리시스(이력 현상)** 를 유발한다는 점입니다.
